...
TEST_CASE("DB Schema", "[wallet,db]")
{
  ...
  {
    REQUIRE_NOTHROW(db.prepared_exec("INSERT INTO blocks VALUES(?,?);", 42, "Adams"));
  ...
  {
    REQUIRE_NOTHROW(db.prepared_exec("INSERT INTO blocks VALUES(?,?);", 0, "foo"));
...
TEST_CASE("DB Triggers", "[wallet,db]")
{
  ...
  REQUIRE_NOTHROW(db.prepared_exec("INSERT INTO outputs VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?);",
        0, 42, 0, 0, 0, false, 0, 0, "out_key", "rct_mask", 0, 0, 0));
  ...
  REQUIRE_NOTHROW(db.prepared_exec("INSERT INTO blocks VALUES(?,?);", 1, "bar"));

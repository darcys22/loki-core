...
namespace wallet
{
  ...
  crypto::public_key Keyring::output_spend_key(const crypto::key_derivation& derivation, const crypto::public_key& output_key, uint64_t output_index)
  {
    ...
    // bool return here is pretty meaningless, but allocate it anyway to not discard its existence entirely
    bool r = key_device.derive_subaddress_public_key(output_key, derivation, output_index, ret);

    return ret;
  }

  std::optional<cryptonote::subaddress_index> Keyring::output_and_derivation_ours(const crypto::key_derivation& derivation, const crypto::public_key& output_key, uint64_t output_index)
  {
  ...
  crypto::key_image Keyring::key_image(const crypto::key_derivation& derivation, const crypto::public_key& output_key, uint64_t output_index, const cryptonote::subaddress_index& sub_index)
  {
